...
export class Agent {
  ...
  private async _ensureRemoteId(): Promise<void> {
    ...
    throw new Error(
      'Remote environment requires a getRemoteId callback or REMOTE_ID environment variable to be set.',
    );
  }
  ...
  public async invokeTool(
    ...
    const result = await withToolCall(
      ...
      sessionState,
      toolResults as any,
      ctx => toolRegistry.executeToolWithCallbacks(toolId, toolUseId, args, ctx),
      {
        executionId,
  ...
  private _setupToolRegistryBridges(): void {
    ...
    this._core.toolRegistry.onToolExecutionComplete(
      (executionId, toolId, toolUseId, args, result, startTime, executionTime) => {
        const { toolName } = resolveToolMeta(toolId);
    ...
    this._core.toolRegistry.onToolExecutionError(
      (executionId, toolId, toolUseId, startTime, args, error) => {
        const { toolName } = resolveToolMeta(toolId);
   ...
   */
  on<E extends BusEventKey>(event: E, handler: (data: BusEvents[E]) => void): () => void {
   ...
   */
  off<E extends BusEventKey>(event: E, handler: (data: BusEvents[E]) => void): void {
